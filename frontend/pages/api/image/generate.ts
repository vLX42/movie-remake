...
export default async function  handler(request: NextRequest) {
  ...
  if (!prompt || !title) {
    return new Response(JSON.stringify({ error: "Prompt and title are required." }), { status: 400 });
  }
  if (!prompt || !title) {
    return NextResponse.json({ error: "Prompt and title are required." }, { status: 400 });
  }

  const imageUrl = await generateImageReplicate(prompt, title);

  if (imageUrl === "error" || imageUrl === "no image") {
    return NextResponse.json({ error: imageUrl }, { status: 500 });
  }
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ imageUrl: "test" });
}